<template>
  <div
    class="city"
    :class="{
      'city--night': isNight(city.sys.sunrise, city.sys.sunset, city.id),
      'city--cloudy': isCloudy(city.weather)
    }"
    :style="colourByTemp(city.main.temp)"
    @keyup.27="toggleDetails()">
    <h2 class="city-temp">{{city.main.temp | tempFormat}}</h2>
    <h1 class="city-name">{{city.name}}<br>{{city.id | countryName}}</h1>
    <button
      class="button city-view-btn"
      @click="toggleDetails(true)">view</button>
    <div
      class="city-details"
      ref="cityDetails"
      :class="{'city-details--opened': opened}"
      :style="colourByTemp(city.main.temp)">
      <button
        class="button city-details-btn"
        @click="toggleDetails()">back</button>
      <div class="city-details-image">
...
export default {
  ...
  methods: {
    ...
    isNight (sunrise, sunset, id) {
      const tzOptions = {
        zone: zones[id]['timezone']
      }
      const now = DateTime.fromMillis(this.dateNow, tzOptions)
      const sunriseTime = DateTime.fromMillis(sunrise * 1000, tzOptions)
      const sunsetTime = DateTime.fromMillis(sunset * 1000, tzOptions)
...
<style lang="scss" src="../styles/city.scss"></style>
